...
Plug 'neoclide/coc.nvim', {'for': ['rust','c','cpp','go', 'javascript','css', 'html', 'json' ],'branch': 'release'} " CocInstall coc-tsserver coc-html coc-css coc-sql coc-go coc-json coc-clangd coc-flutter coc-snippets can also do coc-pyright for python
...
let g:ale_fixers = {'dart': ['dartfmt', 'dart-format'],'javascript': ['prettier','standard', 'eslint' ],'html': ['prettier'], 'css': ['prettier'], 'python':['autopep8'], 'rust': ['rustfmt'], 'go':['gofmt'], 'c':['clang-format', 'clangtidy'],'cpp':['clang-format', 'clangtidy']} " just install prettier for react, eslint already installed by default, also install eslint-config-prettier for good interaction with eslint, rustup component add rustfmt
