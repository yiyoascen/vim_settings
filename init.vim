...
Plug 'neoclide/coc.nvim', {'for': ['rust','c','cpp','go', 'javascript','css', 'html', 'json' ],'branch': 'release'} " CocInstall coc-tsserver coc-html coc-css coc-sql coc-json coc-clangd coc-flutter coc-rust-analyzer can also do coc-pyright for python
...
let g:ale_linters = {'javascript': ['standard', 'eslint'], 'python':['flake8', 'pylint', 'mypy', 'pyright', 'bandit'], 'rust':['cargo'], 'go':['golint','govet'],  'c':['clangd', 'clangcheck'],'cpp':['clangd', 'clangcheck'] } " always install in environment  install clangd, the dart analysis_server causes an error, but all three are needed to fix with ale, coc can handle this
